...
func (ka *rsaKeyAgreement) processClientKeyExchange(config *Config, cert *Certificate, ckx *clientKeyExchangeMsg, version uint16) ([]byte, error) {
	...
	if ka.clientVersion != vers {
		return nil, fmt.Errorf("tls: invalid version in RSA premain (got %04x, wanted %04x)", vers, ka.clientVersion)
	}
...
func (ka *rsaKeyAgreement) generateClientKeyExchange(config *Config, clientHello *clientHelloMsg, cert *x509.Certificate) ([]byte, *clientKeyExchangeMsg, error) {
	...
	sentPreMainSecret := preMainSecret
	if bad == RSABadValueTooLong {
		sentPreMainSecret = make([]byte, len(sentPreMainSecret)+1)
		copy(sentPreMainSecret, preMainSecret)
	} else if bad == RSABadValueTooShort {
		sentPreMainSecret = sentPreMainSecret[:len(sentPreMainSecret)-1]
	}

	encrypted, err := rsa.EncryptPKCS1v15(config.rand(), cert.PublicKey.(*rsa.PublicKey), sentPreMainSecret)
	if err != nil {
		return nil, nil, err
	}
...
type ecdhCurve interface {
	...
	// |publicKey|, and returns agreed-upon |preMainSecret| to the acceptor.
	accept(rand io.Reader, peerKey []byte) (publicKey []byte, preMainSecret []byte, err error)

	// finish returns the computed |preMainSecret|, given the |peerKey|
	// generated by |accept|.
	finish(peerKey []byte) (preMainSecret []byte, err error)
}
...
func (e *ellipticECDHCurve) accept(rand io.Reader, peerKey []byte) (publicKey []byte, preMainSecret []byte, err error) {
	publicKey, err = e.offer(rand)
	if err != nil {
		return nil, nil, err
	}
	preMainSecret, err = e.finish(peerKey)
	if err != nil {
		return nil, nil, err
	}
	return
...
func (e *ellipticECDHCurve) finish(peerKey []byte) (preMainSecret []byte, err error) {
	x, y := elliptic.Unmarshal(e.curve, peerKey)
	if x == nil {
		return nil, errors.New("tls: invalid peer key")
	}
	x, _ = e.curve.ScalarMult(x, y, e.privateKey)
	preMainSecret = make([]byte, (e.curve.Params().BitSize+7)>>3)
	xBytes := x.Bytes()
...
func (e *x25519ECDHCurve) accept(rand io.Reader, peerKey []byte) (publicKey []byte, preMainSecret []byte, err error) {
	publicKey, err = e.offer(rand)
	if err != nil {
		return nil, nil, err
	}
...
func (e *x25519ECDHCurve) finish(peerKey []byte) (preMainSecret []byte, err error) {
	if len(peerKey) != 32 {
		return nil, errors.New("tls: invalid peer key")
	}
...
func (ka *ecdheKeyAgreement) generateClientKeyExchange(config *Config, clientHello *clientHelloMsg, cert *x509.Certificate) ([]byte, *clientKeyExchangeMsg, error) {
	...
	publicKey, preMainSecret, err := ka.curve.accept(config.rand(), ka.peerKey)
	if err != nil {
		return nil, nil, err
	}
