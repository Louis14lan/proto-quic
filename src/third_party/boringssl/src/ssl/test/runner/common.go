...
const (
	...
	extensionExtendedMainSecret       uint16 = 23
...
type ClientSessionState struct {
	...
	mainSecret         []byte              // MainSecret generated by client on a full handshake
	handshakeHash        []byte              // Handshake hash for Channel ID purposes.
	serverCertificates   []*x509.Certificate // Certificate chain presented by the server
	extendedMainSecret bool                // Whether an extended main secret was used to generate the session
	sctList              []byte
	ocspResponse         []byte
	earlyALPN            string
...
type ProtocolBugs struct {
	...
	// RequireExtendedMainSecret, if true, requires that the peer support
	// the extended main secret option.
	RequireExtendedMainSecret bool

	// NoExtendedMainSecret causes the client and server to behave as if
	// they didn't support an extended main secret in the initial
	// handshake.
	NoExtendedMainSecret bool

	// NoExtendedMainSecretOnRenegotiation causes the client and server to
	// behave as if they didn't support an extended main secret in
	// renegotiation handshakes.
	NoExtendedMainSecretOnRenegotiation bool
